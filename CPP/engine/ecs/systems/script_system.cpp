...
void ScriptSystem::update(ComponentStorage& storage, EntityManager& entity_manager, Blackboard& blackboard) {
    ...
    for (Entity entity : scripted_entities) {
        ...
        if (!script.initialized) {
            ...
            std::string filepath = project_paths::assets_dir() + "/scripts/" + script.filename + ".lua";
            ...
            if (load_status != LUA_OK) {
                std::string error = lua_tostring(L, -1);
                lua_pop(L, 1);
                log_error(entity, "failed to load '" + script.filename + ".lua'", error);
                continue;
            }
            ...
            if (lua_pcall(L, 0, 0, 0) != LUA_OK) {
                std::string error = lua_tostring(L, -1);
                lua_pop(L, 1);
                log_error(entity, "failed to execute '" + script.filename + ".lua'", error);
                continue;
            }

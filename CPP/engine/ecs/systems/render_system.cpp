 ...
 * 
 * Renders entities using a priority chain: Images > Color > skip.
...
void RenderSystem::render(const ComponentStorage& storage, const Blackboard& blackboard) {
    ...
    // Sort by entity ID so lower IDs render first (background behind foreground)
    ...
    for (Entity entity : entities) {
        ...
        if (storage.has_component<Images>(entity)) {
            ...
            if (img_opt.has_value()) {
                ...
                draw_entity(x, y, render_width, render_height, Color{0, 0, 0, 255}, texture);
            }

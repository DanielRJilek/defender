...
static int l_get_bullet_fire_request(lua_State* L) {
    Entity id = static_cast<Entity>(luaL_checkinteger(L, 1));
    auto* storage = get_storage(L);
    auto bullet_fire_request = storage->get_component<BulletFireRequest>(id);
    if (!bullet_fire_request.has_value()) {
        lua_pushnil(L);
        return 1;
    }
    lua_pushboolean(L, 1);
...
static int l_set_bullet_fire_request(lua_State* L) {
    Entity id = static_cast<Entity>(luaL_checkinteger(L, 1));
    auto* storage = get_storage(L);
    storage->add_component<BulletFireRequest>(id, BulletFireRequest{});
    return 0;
}
...
static int l_has_component(lua_State* L) {
    ...
    } else if (type == "AstronautTag") {
        result = storage->has_component<AstronautTag>(id);
    } else if (type == "ShipTag") {
        result = storage->has_component<ShipTag>(id);
    } else if (type == "AlienTag") {
        result = storage->has_component<AlienTag>(id);
    } else if (type == "FallingTag") {
        result = storage->has_component<FallingTag>(id);
    } else if (type == "AbductedTag") {
        result = storage->has_component<AbductedTag>(id);
    } else if (type == "AbductingTag") {
        result = storage->has_component<AbductingTag>(id);
    } else if (type == "RescuedTag") {
        result = storage->has_component<RescuedTag>(id);
    } else {
        return luaL_error(L, "Unknown component type: %s", type_name);
    }
...
static int l_entities_with(lua_State* L) {
    ...
    if (type == "Position") {
        ...
        entities = storage->entities_with_component<AstronautTag>();
    } else if (type == "ShipTag") {
        entities = storage->entities_with_component<ShipTag>();
    } else if (type == "AlienTag") {
        entities = storage->entities_with_component<AlienTag>();
    } else if (type == "FallingTag") {
        entities = storage->entities_with_component<FallingTag>();
    } else if (type == "AbductedTag") {
        entities = storage->entities_with_component<AbductedTag>();
    } else if (type == "AbductingTag") {
        entities = storage->entities_with_component<AbductingTag>();
    } else if (type == "RescuedTag") {
        entities = storage->entities_with_component<RescuedTag>();
    } else {
        return luaL_error(L, "Unknown component type: %s", type_name);
    }
...
static int l_get_entity_count(lua_State* L) {
    // No arguments needed
    // Retrieve the EntityManager from the registry
    auto* em = get_entity_manager(L);
    // Call active_count() to get the number of active entities
    int count = em->active_count();
    // Push the count as an integer
...
void register_bindings(lua_State* L) {
    ...
    lua_pushcfunction(L, l_get_bullet_fire_request);
    lua_setfield(L, -2, "get_bullet_fire_request");
    lua_pushcfunction(L, l_set_bullet_fire_request);
    lua_setfield(L, -2, "set_bullet_fire_request");
    ...
    lua_pushcfunction(L, l_get_entity_count);
    lua_setfield(L, -2, "get_entity_count");

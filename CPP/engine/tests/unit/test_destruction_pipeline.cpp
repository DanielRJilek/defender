 ...
 *
 * Requirements tested: 13.1, 13.2, 13.3, 13.4, 1.3, 2.4, 3.2, 5.2, 5.3
 */
...
TEST_CASE("Only marked entities are destroyed; unmarked entities are untouched", "[destruction_pipeline]") {
    ...
    CHECK(p1->get().x == Catch::Approx(1.0f));
    CHECK(p1->get().y == Catch::Approx(1.0f));
    auto s1 = storage.get_component<Size>(e1);
    REQUIRE(s1.has_value());
    CHECK(s1->get().width == Catch::Approx(10.0f));
    CHECK(s1->get().height == Catch::Approx(10.0f));
    ...
    CHECK(p3->get().x == Catch::Approx(3.0f));
    CHECK(p3->get().y == Catch::Approx(3.0f));
    auto s3 = storage.get_component<Size>(e3);
    REQUIRE(s3.has_value());
    CHECK(s3->get().width == Catch::Approx(30.0f));
    CHECK(s3->get().height == Catch::Approx(30.0f));
}
...
TEST_CASE("Zero marked entities leaves everything unchanged", "[destruction_pipeline]") {
    ...
    auto p1 = storage.get_component<Position>(e1);
    REQUIRE(p1.has_value());
    CHECK(p1->get().x == Catch::Approx(10.0f));
    CHECK(p1->get().y == Catch::Approx(10.0f));

    auto p2 = storage.get_component<Position>(e2);
    REQUIRE(p2.has_value());
    CHECK(p2->get().x == Catch::Approx(20.0f));
    CHECK(p2->get().y == Catch::Approx(20.0f));

    auto p3 = storage.get_component<Position>(e3);
    REQUIRE(p3.has_value());
    CHECK(p3->get().x == Catch::Approx(30.0f));
    CHECK(p3->get().y == Catch::Approx(30.0f));
}

TEST_CASE("Entity with all component types has every one removed", "[destruction_pipeline]") {
    EntityManager em;
    ComponentStorage storage;
    ...
    storage.add_component(entity, Splittable{AsteroidTier::Medium});
    storage.add_component(entity, DestroyRequest{});
    ...
    CHECK_FALSE(storage.has_component<Splittable>(entity));
    CHECK_FALSE(storage.has_component<DestroyRequest>(entity));
}

TEST_CASE("Default-constructed Rotation has zeroed fields", "[destruction_pipeline]") {
    Rotation r{};
    CHECK(r.angle == Catch::Approx(0.0f));
    CHECK(r.angular_velocity == Catch::Approx(0.0f));
}
...
TEST_CASE("Default-constructed Lifetime has remaining=0.0f", "[destruction_pipeline]") {
    Lifetime l{};
    CHECK(l.remaining == Catch::Approx(0.0f));
}

TEST_CASE("Default-constructed Splittable has tier=AsteroidTier::Large", "[destruction_pipeline]") {
    Splittable s{};
    CHECK(s.tier == AsteroidTier::Large);
}

TEST_CASE("AsteroidTier has three distinct values", "[destruction_pipeline]") {
    auto large  = static_cast<uint8_t>(AsteroidTier::Large);
    auto medium = static_cast<uint8_t>(AsteroidTier::Medium);
    auto small  = static_cast<uint8_t>(AsteroidTier::Small);

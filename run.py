...
def ctest_run(filter_re=None, test_dir="build", extra_args=None, exclude_re=None):
    cmd = ["ctest", "--test-dir", test_dir, "--output-on-failure", "-C", "Debug"]
    if filter_re:
        cmd += ["-R", filter_re]
    if exclude_re:
        cmd += ["-E", exclude_re]
...
def _print_timeline(session):
    ...
    type_order = {"KEY": 0, "DUMP": 1, "TRACE": 2, "STOP": 3}
    ...
    while True:
        ...
        try:
            ...
            if choice == 0:  # Compile
                ...
                cmake_configure(debug=debug_build)
                cmake_build()
                ctest_run("^Engine", exclude_re="Timer")
                ...
                cmake_configure(debug=debug_build)
                cmake_build()
                ctest_run(test_dir="build/tests", exclude_re="Timer")

            elif choice == 6:  # All Tests (with Timer)
                cmake_configure(debug=debug_build)
                cmake_build()
                ctest_run(test_dir="build/tests")

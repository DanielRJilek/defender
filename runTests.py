...
def main():
    ...
    subprocess.run(
        ["ctest", "--test-dir", "build", "-R", "^Engine", "--output-on-failure", "-C", "Debug"]
        + exclude_args,
        cwd=cpp_dir,
        check=True,
    )
    ...
    print("\n=== Running Game Tests ===")
    subprocess.run(
        ["ctest", "--test-dir", "build", "-R", "^Game", "--output-on-failure", "-C", "Debug"],
        cwd=cpp_dir,
        check=True,
    )
    ...
    if args.plusTimer:
        print("\n=== Running Timer Tests ===")
        subprocess.run(
            ["ctest", "--test-dir", "build", "-R", "Timer", "--output-on-failure", "-C", "Debug"],
            cwd=cpp_dir,
            check=True,
        )
